...
public class Login extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			}else if (em.equalsIgnoreCase("RECORDS@GMAIL.COM") && pa.equalsIgnoreCase("RECORDS")) {
					session.setAttribute("uid", 6);
					session.setAttribute("Role", 6);
					
					out.print("<html><body><script>alert('Login Successful !!');</script></body></html>");
					
					RequestDispatcher rd = request.getRequestDispatcher("Home_RECORDS.jsp");
					rd.forward(request, response);
			}
			else {
				System.out.println(em);
